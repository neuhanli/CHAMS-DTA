...
def create_dataset(dataset,fold=0):
    ...
    valid_fold = train_fold[fold]
    train_fold = [ee for e in (train_fold[:fold] + train_fold[fold+1:]) for ee in e]
    ...
    for opt in opts:
        ...
        with open('data/' + dataset + '_fold' + str(fold) + '_' + opt + '.csv', 'w') as f:
    ...
    df_train = pd.read_csv('data/' + dataset + '_fold' + str(fold) + '_train.csv')
    ...
    df_test = pd.read_csv('data/' + dataset + '_fold' + str(fold) + '_test.csv')
    ...
    train_data = TestbedDataset(root='data', dataset=dataset+'_fold'+str(fold)+'_train', xd=train_drugs, xt=train_prots,
                                y=train_Y, smile_tensor=smile_tensor)
    test_data = TestbedDataset(root='data', dataset=dataset+'_fold'+str(fold)+'_test', xd=test_drugs, xt=test_prots,
