...
for dataset in datasets:
    for fold in range(5):
        print('\nrunning on ', model_st + '_' + dataset + '_fold' + str(fold))

        start_time = time.time()
        train_data_file = "./data/" + dataset + "_traindata_fold" + str(fold) + "_" + str(TRAIN_BATCH_SIZE) + ".data"
        test_data_file = "./data/" + dataset + "_testdata_fold" + str(fold) + "_" + str(TEST_BATCH_SIZE) + ".data"
        if not (os.path.isfile(train_data_file) and os.path.isfile(test_data_file)):
            train_data, test_data = create_dataset(dataset, fold)
        ...
        checkpoint_path = 'checkpoint_davis_fold' + str(fold) + '.pt'
        startEpoch = 0
        ...
        rm2 = 0
        best_epoch = -1
        model_file_name = 'model_' + model_st + '_' + dataset + '_fold' + str(fold) + '.model'
        result_file_name = 'result_' + model_st + '_' + dataset + '_fold' + str(fold) + '.csv'
        ...
        for epoch in range(startEpoch, NUM_EPOCHS):
            ...
            if ret[0] < best_mse:
                ...
                best_ci = ret[1]
                rm2 = ret[2]
                print('mse improved at epoch ', best_epoch, '; best_mse,best_ci,rm2:', best_mse, best_ci, rm2, model_st, dataset, 'fold', fold)
            elif (epoch - best_epoch)<200:
                print(ret[0], 'No improvement since epoch ', best_epoch, '; best_mse,best_ci,rm2:', best_mse, best_ci, rm2, model_st, dataset, 'fold', fold)
            else:
                print('early stop  ''; best_mse,best_ci,rm2:', best_mse, best_ci, rm2, model_st, dataset, 'fold', fold)
                break
